...
export function RoundProvider({ children }: { children: ReactNode }) {
  ...
  const loadRound = useCallback(async () => {
    ...
    const { data: roundData } = await supabase
      .from("rounds")
      .select(
        "id,theme,theme_description,theme_author,status,season_number,round_number,submission_deadline,voting_deadline,playlist_url,external_playlist_url,theme_image_url"
      )
